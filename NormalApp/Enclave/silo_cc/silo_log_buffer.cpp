...
void LogBuffer::push(std::uint64_t tid, NotificationId &nid, std::vector<WriteElement> &write_set) {
    ...
    for (auto &itr : write_set) {
        log_set_[log_set_size_++] = LogRecord(tid, 
                                              itr.key_.uint64tToString(itr.key_.slices, itr.key_.lastSliceSize), 
                                              itr.get_new_value()->body_); // TODO: ここがちゃんと動作するか確認
    }
